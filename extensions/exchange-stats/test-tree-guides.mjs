...
test("settled progress guides every row across Pi components and keeps the final answer outside", () => {
	...
	const messages = [
		...
		snapshot(1802, [{ type: "text", text: "First note." }, { type: "text", text: "Second note." }]),
		...
		snapshot(1805, [{ type: "toolCall", id: "guide-last", name: "bash", arguments: { command: "run.sh" } }]),
	...
	try {
		const components = [
			...
			assistant(messages[3]), tool("guide-last", "bash", { command: "run.sh" }, "hello from script"),
		...
		assert.match(rendered[progressAt].trim(), /◈ 2 ⚙ 2 · 3 notes$/);
